...
$cellVCentered = array('valign' => 'center');
    ...
    if ($tipo_peo=="Estudiante") {
        if ($ident=="") {
            $rspta = $obj->listarstudent($date_star,$date_end,$tipo_peo,$datos1_peo,$datos2_peo,$anio_peoget);
        } else {
           $rspta = $obj->listar_ident($date_star,$date_end,$tipo_peo,$anio_peoget,$ident);
       }
    } else {

        if ($ident=="") {
            $rspta = $obj->listarall($date_star,$date_end,$tipo_peo,$anio_peoget);
        } else {
           $rspta = $obj->listar_ident($date_star,$date_end,$tipo_peo,$anio_peoget,$ident);
       }
    }
            ...
            $table->addCell()->addText($reg->$time_end,array('color' => '#000000'),$cellHCentered);
