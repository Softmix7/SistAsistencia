...
$cellVCentered = array('valign' => 'center');
    ...
    if ($tipo_peo=="Estudiante") {
        if ($ident=="") {
            $rspta = $obj->listarstudent($date_star,$date_end,$tipo_peo,$datos1_peo,$datos2_peo,$timein,$anio_peoget);
        } else {
           $rspta = $obj->listar_ident($date_star,$date_end,$tipo_peo,$anio_peoget,$timein,$ident);
       }
    } else {

        if ($ident=="") {
            $rspta = $obj->listarall($date_star,$date_end,$tipo_peo,$timein,$anio_peoget);
        } else {
           $rspta = $obj->listar_ident($date_star,$date_end,$tipo_peo,$anio_peoget,$timein,$ident);
       }
    }
            ...
            $table->addRow();
            $table->addCell(null, $cellColor5)->addText('PERSONAL',array('color' => 'FFFFFF'),$cellHCentered);
			...
			$table->addCell(NULL,array('borderBottomColor' => 'FFFFFF', 'bgColor' => '006B3D'))->addText('DATOS1',array('color' => 'FFFFFF','size' => 8),$dafault);
			$table->addCell(NULL,array('borderBottomColor' => 'FFFFFF', 'bgColor' => '006B3D'))->addText('DATOS2',array('color' => 'FFFFFF','size' => 8),$dafault);
            ...
            $table->addCell()->addText($reg->lastname_peo,array('color' => '000000'),$dafault);
            $table->addCell()->addText($reg->name_peo,array('color' => '000000'),$dafault);
            $table->addCell()->addText($reg->datos1_peo,array('color' => '#000000'),$dafault);
            $table->addCell()->addText($reg->datos2_peo,array('color' => '#000000'),$dafault);
            ...
            if ($tardanza=="Temprano") {
				...
				$table->addCell()->addText($tardanza,array('color' => '#087505'),$cellHCentered);
			}else if($tardanza=="Retardo ". $reg->tardanza){
				$table->addCell()->addText($tardanza,array('color' => '#E9841F'),$cellHCentered);
